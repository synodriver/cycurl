...
def test_cookies_mislead_by_host(server):
    s = requests.Session(debug=True)
    s.curl.setopt(CURLOPT_RESOLVE, ["example.com:8008:127.0.0.1"])
...
def test_cookies_redirect_to_another_domain(server):
    s = requests.Session()
    s.curl.setopt(CURLOPT_RESOLVE, ["google.com:8008:127.0.0.1"])
...
def test_curl_infos(server):
    ...
    assert r.infos[CURLINFO_PRIMARY_PORT] == 8008
