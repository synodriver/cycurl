...
class FingerprintUpdateError(RuntimeError, m.CurlError):
...
class FingerprintManager:
    ...
    def _fetch_fingerprint_payload(url: str, headers: dict[str, str]) -> bytes:
        ...
        buffer = BytesIO()
        try:
            curl.setopt(m.CURLOPT_URL, url.encode())
            if headers:
                curl.setopt(
                    m.CURLOPT_HTTPHEADER,
                    [f"{key}: {value}".encode() for key, value in headers.items()],
                )
            curl.setopt(m.CURLOPT_WRITEDATA, buffer)
            curl.perform()
            status_code = int(curl.getinfo(m.CURLINFO_RESPONSE_CODE))
