...
from cycurl.requests.utils import NOT_SET, NotSetType, set_curl_options
...
if TYPE_CHECKING:
    ...
    SEND_QUEUE_ITEM = tuple[bytes | bytearray | memoryview, int]
...
class WebSocketRetryStrategy:
    ...
    Args:
        ...
        codes: Set of ``CURLE`` values for which the receive operation
            should be retried. Default is ``CURLE_RECV_ERROR``.
    """

    retry: bool = False
    delay: float = 0.3
    count: int = 3
    codes: set[int] = field(default_factory=lambda: {m.CURLE_RECV_ERROR})
...
class WebSocketError(CurlError):
    ...
    def __init__(self, message: str, code: WsCloseCode | int | Literal[0] = 0) -> None:
...
class WebSocket(BaseWebSocket):
    ...
    def _emit(
        ...
        *args: str | bytes | int | WSFrame | CurlError,
    ) -> None:
    ...
    def connect(
        ...
        http_version: int | None = None,
        interface: str | None = None,
        cert: str | tuple[str, str] | None = None,
        max_recv_speed: int = 0,
        curl_options: dict[int, str] | None = None,
    ) -> Self:
        ...
        _ = curl.setopt(m.CURLOPT_CONNECT_ONLY, 2)
        curl.perform()
        return self
    ...
    def recv(self) -> tuple[bytes, int]:
        ...
        while True:
            try:
                ...
                if frame.flags & (m.CURLWS_PING | m.CURLWS_PONG):
                    continue
    ...
    def close(self, code: int = WsCloseCode.OK, message: bytes = b""):
        ...
        _ = self.send(msg, m.CURLWS_CLOSE)
...
class AsyncWebSocket(BaseWebSocket):
    ...
    def _start_io_tasks(self) -> None:  # pyright: ignore[reportUnusedFunction]
        ...
        self._sock_fd = cast(int, self.curl.getinfo(m.CURLINFO_ACTIVESOCKET))
        if self._sock_fd == m.CURL_SOCKET_BAD:
            raise WebSocketError(
                "Invalid active socket.", code=m.CURLE_NO_CONNECTION_AVAILABLE
            )
    ...
    async def ping(self, payload: str | bytes) -> None:
        ...
        return await self.send(payload_bytes, m.CURLWS_PING)
        ...
        async with self._close_lock:
            ...
            try:
                if (
                    ...
                    await asyncio.wait_for(
                        self._send_queue.put((close_frame, m.CURLWS_CLOSE)),
                        timeout=timeout,
                    )
    ...
    async def _read_loop(self) -> None:
        ...
        curl_ws_recv: Callable[[], tuple[bytes, WSFrame]] = self.curl.ws_recv
        ...
        retry_codes: set[int] = self.ws_retry.codes
        max_retries: int = self.ws_retry.count
        retry_base: float = float(self.ws_retry.delay)
        e_again: int = m.CURLE_AGAIN
        e_recv_err: int = m.CURLE_RECV_ERROR
        e_nothing: int = m.CURLE_GOT_NOTHING
        close_flag: int = m.CURLWS_CLOSE
        cont_flag: int = m.CURLWS_CONT
        data_mask: int = m.CURLWS_BINARY | m.CURLWS_TEXT | cont_flag
            ...
            while not self.closed:
                        ...
                        # pylint: disable-next=broad-exception-caught
                            ...
                            self._finalize_connection(
                                WebSocketError(
                                    ...
                                    m.CURLE_NO_CONNECTION_AVAILABLE,
                                )
                            )
                            return
                    ...
                    if not (flags & cont_flag or frame.bytesleft):
                        ...
                        try:
                            ...
                            if not block_on_recv:
                                self._finalize_connection(
                                    WebSocketError(
                                        queue_full_err, m.CURLE_OUT_OF_MEMORY
                                    )
                                )
                                return
                ...
                if flags & close_flag:
                    ...
                    try:
                        ...
                        if not block_on_recv:
                            self._finalize_connection(
                                WebSocketError(queue_full_err, m.CURLE_OUT_OF_MEMORY)
                            )
                            return
    ...
    async def _write_loop(self) -> None:
        ...
        """
        control_frame_flags: int = m.CURLWS_CLOSE | m.CURLWS_PING | m.CURLWS_PONG
    ...
    async def _send_payload(
        self, payload: bytes | memoryview | bytearray, flags: int
    ) -> bool:
        """
        Optimized low-level sender with fragmentation logic.
        """
        # Cache locals to reduce lookup cost
        curl_ws_send: Callable[[memoryview, int], int] = self.curl.ws_send
        loop: asyncio.AbstractEventLoop = self.loop
        loop_time: Callable[[], float] = loop.time
        create_future: Callable[[], asyncio.Future[None]] = loop.create_future
    ...
    async def _handle_close_frame(self, message: bytes) -> None:
        ...
        if self.autoclose and not self.closed:
            close_code: int | Literal[WsCloseCode.OK] = (
                WsCloseCode.OK
