...
ON_ERROR_T = Callable[["WebSocket", int], None]
...
class WebSocketError(m.CurlError):
    pass
...
class WebSocket:
    ...
    def send(self, payload: bytes, flags: int = m.CURLWS_BINARY):
        """Send a data frame"""
    ...
    async def asend(self, payload: bytes, flags: int = m.CURLWS_BINARY):
