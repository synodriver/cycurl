...
if uname.system == "Windows":
    library_dirs = ["./dep/libcurl-impersonate-v1.1.1.x86_64-win32/lib"]
    extra_objects = []
    for obj in glob.glob("./dep/libcurl-impersonate-v1.1.1.x86_64-win32/lib/*.lib"):
        extra_objects.append(obj)
    extra_objects = [
        i for i in extra_objects if "libcurl.lib" not in i
    ]  # only libcurl_imp is enough
    for file in glob.glob("./dep/libcurl-impersonate-v1.1.1.x86_64-win32/bin/*.dll"):
        shutil.copy(file, "./cycurl")
elif uname.system == "Darwin":
    if platform.machine() == "x86_64":
        library_dirs = ["./dep/libcurl-impersonate-v1.1.1.x86_64-macos"]
        extra_objects = [
            "./dep/libcurl-impersonate-v1.1.1.x86_64-macos/libcurl-impersonate.4.dylib"
        ]
        for file in glob.glob(
            "./dep/libcurl-impersonate-v1.1.1.x86_64-macos/*.dylib"
        ):
            shutil.copy(file, "./cycurl")
    else:
        library_dirs = ["./dep/libcurl-impersonate-v1.1.1.arm64-macos"]
        extra_objects = [
            "./dep/libcurl-impersonate-v1.1.1.arm64-macos/libcurl-impersonate.4.dylib"
        ]
        for file in glob.glob(
            "./dep/libcurl-impersonate-v1.1.1.arm64-macos/*.dylib"
        ):
            shutil.copy(file, "./cycurl")
else:
    library_dirs = ["./dep/libcurl-impersonate-v1.1.1.x86_64-linux-gnu"]
    extra_objects = [
        "./dep/libcurl-impersonate-v1.1.1.x86_64-linux-gnu/libcurl-impersonate.so.4.8.0"
    ]
    for file in glob.glob("./dep/libcurl-impersonate-v1.1.1.x86_64-linux-gnu/*.so"):
        shutil.copy(file, "./cycurl")
